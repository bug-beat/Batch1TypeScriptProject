...
// STEP 6: Check the cancellation flexibility priority: 
// if cancellation flexibility is "strict" accept all, 
// if "partial" accept only "free" and "partial", 
// if "free" accept only "free"
...
// This method takes two arguments: minimum discount percentage as a whole number, 
// and maximum days until check-in as a whole number
// This method gives back a list of matching deal descriptions (multiple pieces of text)
// STEP 1: Inside the method, create an array with at least 8 different deals with discount
//  percentage, deal description, and days until check-in
...
// STEP 7: If BOTH the discount percentage meets requirement AND the availability timeframe is acceptable, 
// add the deal description to your matching array
...
class BookingComHomePage {
    // STEP 5: Check if the amenity part equals the required amenity
    // STEP 6: Check the cancellation flexibility priority: 
    // if cancellation flexibility is "strict" accept all, (just push it)

    // if "partial" accept only "free" and "partial", 
    // (cancellationFlexibility === "partial") 
    // (currentArray[2] === 'free' || currentArray[2] === 'partial')

    // if "free" accept only "free"
    // (cancellationFlexibility === "free") 
    // (currentArray[2] === 'free')

    // STEP 7: If BOTH the amenity matches AND the cancellation policy is acceptable, add the property name to your matching array
    // STEP 8: After the loop finishes, give back the array of matching property names

    findPropertiesByAmenitiesAndCancellationPolicy(requiredAmenity: string, cancellationFlexibility: string): string[] {
        let listOfAmenitiesHotelsCancellation = [
        ...
        for (let i = 0; i < listOfAmenitiesHotelsCancellation.length; i++) {
            let currentArray = listOfAmenitiesHotelsCancellation[i].split('-')

            if (requiredAmenity === currentArray[0]) {
                // 3 conditions here
                if (cancellationFlexibility === 'strict') {
                    filteredProperties.push(currentArray[1])
                } else if (cancellationFlexibility === 'partial' && ['partial', 'free'].includes(currentArray[2])) {
                    filteredProperties.push(currentArray[1])
                } else if (cancellationFlexibility === 'free' && currentArray[2] === 'free') {
                    filteredProperties.push(currentArray[1])
                }
            }
        }
